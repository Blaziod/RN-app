...
import Earn1FSMenu from '../../Components/Menus/earn1FSMenu';
import Earn1CustomSwitch from '../../Components/CustomSwitches/earn1CustomSwitch';
import {Path, Svg, Stop, Defs, RadialGradient} from 'react-native-svg';
  ...
  const fetchAvailableTasks = async () => {
    ...
    if (userAccessToken) {
      try {
        ...
        if (response.ok) {
          setAvailableTasks(data.goals.Follow);
          setIsLoading(false);
          console.log('Tasks:', data.goals.Follow);
        } else {
